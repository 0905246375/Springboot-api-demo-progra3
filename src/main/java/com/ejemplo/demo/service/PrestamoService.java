...
public class PrestamoService {
    ...
    public PrestamoResponse simular(PrestamoRequest request) {
        ...
        BigDecimal numerador = monto
                .multiply(tasaMensual, MATH_CONTEXT)
                .multiply(BigDecimal.valueOf(factor), MATH_CONTEXT);

        BigDecimal denominador = BigDecimal
                .valueOf(factor)
    ...
    private void validarReglas(PrestamoRequest request) {
        if (request.tasaAnual().compareTo(BigDecimal.valueOf(100)) > 0) {
            throw new IllegalArgumentException(
                    "La tasaAnual no puede ser mayor al 100%"
            );
        }
